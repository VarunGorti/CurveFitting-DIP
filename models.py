...
class BAYESIAN_RES_UNET(nn.Module):
    def __init__(self, bs, nz, ngf=64, output_size=1024, nc=1, optimize_z=False, kernel_size=3, num_layers=None, use_skip=True, causal_passive=False, p_dropout=0.0):
        """
        Args:
            bs: the batch size
        ...
        for l in range(self.num_layers - 1):
                if l == 0:
                    ...
                    self.encoder.append(
                        BayesianResidualConv(in_channels=self.ngf[l-1], out_channels=self.ngf[l], kernel_size=self.kernel_size[l], downsample=True, use_skip = self.use_skip, p_dropout=self.p_dropout)
                    )
                self.decoder.append(
                    BayesianResidualConv(in_channels=2*self.ngf[l], out_channels=self.ngf[l], kernel_size=self.kernel_size[l], downsample=False, use_skip = self.use_skip, p_dropout=self.p_dropout)
                )
                self.upsamples.append(
